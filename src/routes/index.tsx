...
function HomePage() {
  ...
  const [voiceSession, setVoiceSession] = useState<(VoiceEmergencyResult & { speech: string }) | null>(null);
  ...
  async function handleVoiceSOS(text: string) {
    ...
    try {
      ...
      setTimeout(() => { voiceArmed.current = false; }, 1500);
    }
  }
    ...
    <main className="relative">
      <section className="mx-auto flex max-w-5xl flex-col items-center px-5 pb-20 pt-8 text-center sm:pt-14">
        ...
        <div className="mt-6 w-full max-w-sm">
          <GpsIndicator state={state} onRetry={verify} threshold={threshold} locationLabel={locationLabel} />
        </div>
      ...
      {picker && (
        <EmergencyTypeModal
          ...
          onSelect={(c) => { setPicker(false); setNormalActive(c); }}
        />
      )}
