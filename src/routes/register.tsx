  ...
  head: () => ({
    meta: [
      { title: "Become a Hero — ResQNear" },
      {
        name: "description",
        content:
          "Join 24/7 trained heroes saving lives in your neighbourhood. Register in under a minute.",
      },
    ],
  }),
...
function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem(
        "resqnear_hero",
        JSON.stringify({ name: form.name, skill: form.skill, locality: form.locality }),
      );
      setTimeout(() => {
        setSubmitted(false);
  ...
  const heroData = localStorage.getItem("resqnear_hero")
    ? JSON.parse(localStorage.getItem("resqnear_hero")!)
    : null;
  if (heroData) {
    return (
      <main className="mx-auto max-w-xl px-5 pb-20 pt-8 text-center">
        ...
        <div className="flex flex-col gap-3 max-w-xs mx-auto">
          <a
            href="/"
            className="rounded-2xl bg-gradient-blue-violet px-6 py-3 text-sm font-bold text-white text-center shadow-glow-blue"
          >
            ← Back to Home
          </a>
          <button
            ...
            className="rounded-2xl border border-white/20 px-6 py-3 text-sm font-bold text-white/70"
          >
            Update Registration
          </button>
        </div>
    ...
    <main className="mx-auto max-w-xl px-5 pb-20 pt-8">
      ...
      <form onSubmit={onSubmit} className="mt-8 space-y-4 rounded-3xl glass-card p-6">
        ...
        <Field label="Skill">
          ...
          >
            {SKILLS.map((s) => (
              <option key={s} value={s} className="bg-[#16213E]">
                {s}
              </option>
            ))}
          </select>
        ...
        <label className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
          ...
          >
            <span
              className={`absolute top-0.5 h-6 w-6 rounded-full bg-white shadow transition ${form.available ? "left-[22px]" : "left-0.5"}`}
            />
          </button>
        </label>
        ...
        <button
          type="submit"
          disabled={saving}
          className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-blue-violet px-5 py-4 text-base font-extrabold uppercase tracking-[0.22em] text-white shadow-glow-blue transition hover:scale-[1.01] active:scale-[0.98] disabled:opacity-70"
        >
          {saving ? (
            <>
          ...
          <div className="fixed inset-0 z-50 grid place-items-center bg-[#0F0F1A]/80 backdrop-blur-md p-4">
            <div className="animate-fade-up w-full max-w-sm rounded-3xl glass-card p-8 text-center">
              ...
              <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-white">
                Welcome, {savedName.split(" ")[0] || "Hero"} 🎉
              </h2>
              <p className="mt-1 text-sm text-muted-foreground">
                You're now part of India's 24/7 hero network. We'll alert you when someone nearby
                needs help.
              </p>
              <button
                onClick={() => {
                ...
                className="mt-5 w-full rounded-2xl bg-gradient-blue-violet px-5 py-3 text-sm font-extrabold uppercase tracking-widest text-white shadow-glow-blue"
              >
                Go to Home
              </button>
            </div>
