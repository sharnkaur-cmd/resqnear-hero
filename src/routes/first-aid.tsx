  ...
  head: () => ({
    meta: [
      { title: "AI First-Aid Guide — ResQNear" },
      {
        name: "description",
        content:
          "Step-by-step first-aid for cardiac arrest, fire, accidents, choking and more. Built for India.",
      },
    ],
  }),
    ...
    <main className="mx-auto max-w-3xl px-5 pb-20 pt-8">
      <header className="text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Pick the situation. Follow the steps. We'll read them aloud.
        </p>
      </header>
      ...
      <div className="mt-6 rounded-3xl glass-card p-5">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={toggleSpeak}
              className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-gradient-blue-violet px-4 py-2.5 text-sm font-bold text-white shadow-glow-blue"
            >
              {speaking ? (
                <>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {EMERGENCY_NUMBERS.map((n) => (
            <a
              key={n.code}
              href={`tel:${n.code}`}
              className="group rounded-2xl glass-card p-4 text-center transition hover:-translate-y-0.5 hover:border-white/20"
            >
              <Phone className="mx-auto h-4 w-4 text-[#4cc9f0]" />
              <div className="mt-1 text-3xl font-extrabold tracking-tight text-gradient-primary">
                {n.code}
              </div>
