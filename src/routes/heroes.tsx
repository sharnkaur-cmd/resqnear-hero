...
const FILTERS = ["All", "Medical", "Safety", "Nearby"] as const;
type Filter = (typeof FILTERS)[number];

const PODIUM = [
  { color: "#C0C0C0", glow: "0 0 30px rgba(192,192,192,0.55)", h: "h-32", label: "Silver", rank: 2 },
  { color: "#FFD700", glow: "0 0 40px rgba(255,215,0,0.75)", h: "h-44", label: "Gold", rank: 1 },
...
function HeroesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "Medical") return HEROES.filter((h) => ["Doctor", "Nurse", "Paramedic"].includes(h.skill));
    if (filter === "Safety") return HEROES.filter((h) => h.skill === "CPR Trained" || h.skill === "First Aider");
    return [...HEROES].sort((a, b) => a.distanceM - b.distanceM);
  }, [filter]);
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="flex items-center gap-2 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><MapPin className="h-3 w-3" /> {h.area}</span>
                  <span className="flex items-center gap-1"><Timer className="h-3 w-3" /> {h.avgResponse}</span>
                </p>
              ...
              <div className="text-right">
                ...
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground">lives helped</div>
              </div>
            </div>
          );
        })}
